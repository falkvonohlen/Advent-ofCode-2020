...
impl Passport {
    ...
    fn parse_batch_file(input: Vec<String>) -> Vec<Passport> {
        let mut passports = vec![];
        let mut next_line: usize = 0;

        while next_line < input.len() {
            let unparsed_input = &input[next_line..];
            let empty_line = unparsed_input.iter().position(|l| l.as_str() == "");
            let end = match empty_line {
                Some(num) => num + next_line,
                None => input.len(),
            };
            let one_passport_lines = &input[next_line..end];
            passports.push(Passport::parse_passport_batch(one_passport_lines));
            next_line = end + 1;
        }
        passports
    }
...
pub fn part1() {
    let input_file =
        load_input::load_strings("./resources/Day4Input.txt").expect("Failed to read the input");

    let ports = Passport::parse_batch_file(input_file);
...
mod tests {
    ...
    fn test_parse_batch_file() {
        ...
        let ports = Passport::parse_batch_file(input);
        assert_eq!(
            2,
            ports
                .into_iter()
